...
#define PLUGIN 		"Furien : Bomb Events"
#define VERSION 	"1.5"
...
{
	"sound/fmu_sounds/fmu_bombplanted.mp3",
	"sound/fmu_sounds/fmu_bombplanted2.mp3"
};
...
{
	precache_generic( g_szBombPlantedSounds[0] );
	precache_generic( g_szBombPlantedSounds[1] );
}
...
public bomb_planted( ) 
{
	...
	if( get_pcvar_num( gCvarSound ) > 0 )
	{
		...
		client_cmd( 0, "mp3 play ^"%s^"", g_szBombPlantedSounds[ g_iSound ] );
...
public ShowTimeUntilExplosion(  )
{
	if( g_C4Timer > 0 )
	{
		...
		ShowSyncHudMsg( 0, gSyncHudMessage, "Bomb explodes in: %i Seconds !!", g_C4Timer, g_C4Timer == 1 ? "a" : "e" );
		g_C4Timer--;
	}
...
public Hook_TextMessages( iMsgId, iMsgDest, id )
{
	...
	if( iDropped && equal( szMsg, "#Game_bomb_drop" ) )
	{
		...
		ShowSyncHudMsg( 0, gSyncHudMessage, "Furiens lost the Bomb!" );
	...
	{
		set_hudmessage( 255, 0, 0, -1.0, 0.16, 0, 0.0, 3.5, 0.1, 0.1, -1 );
		ShowSyncHudMsg( 0, gSyncHudMessage, "Furiens recovered the Bomb!" );
		return PLUGIN_HANDLED;
	}
