...
public PostSay(id, isTeamMsg, szMsg[])
{
	...
	ClientColourSay( iTeam, isTeamMsg, "^x01%s%s^x03[Level %d] ^x04[*%s*] ^x03%s ^x01: %s%s", szTeam, szStatus, sk_get_user_level(id), gszAdminTags[g_Admin[id]], szName, ((g_Admin[id]<4)?"^x04":""), szMsg);
}
